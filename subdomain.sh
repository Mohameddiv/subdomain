...
rsescan -d $1 -key 45caaddd-b3fd-45fa-b70f-3f03ed9eaa81 | grep "$1"  | anew |sort >> $Dir/$1.txt
...
#    -H $'Host: www.dnsgrep.cn' -H $'User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:128.0) Gecko/20100101 Firefox/128.0' \
#    -b $'cf_clearance=rcLouItY8p9UnP3TFM7huza_wDRD_879yp..31tRO7o-1758386794-1.2.1.1-4hZoA2jfNzD85dfnt7v3Yn4uJP_duv7wQgc4bzQsLOOYNpSlAswoDu4yiMhNv96J4UkrmUOVwpy2FMDCkEsMxvP1shtvWzRsGjHZ14VX_JuNL.c82rCY72V3cWlEqWdT6nlECZ9tSSUO2A7RTjrem6YX7Ooa1A.7tZL4FAI_eds1ttiaCsrbQFoqzEUdLU1w3RoaU6vY1X5jMHbYToR7GqiNrolwFaecJlpOSX0isWs' \
#    $"https://www.dnsgrep.cn/subdomain/$1" |grep -Po "((http|https):\/\/)?(([\w.-]*)\.([\w]*)\.([A-z]))\w+" | sort -u| anew |sort >> $Dir/$1.txt
...
curl https://api.securitytrails.com/v1/domain/$1/subdomains --header 'APIKEY: hJp42gwvt8TpbzTqp7TqpN9_AzJdznmU'| jq .subdomains 2>/dev/null | awk -v domain=$1 -F\" '{print $2 "." domain}' | sed '$d' | sed '1d' | anew |sort >> $Dir/$1.txt
curl "https://sub-scan-api.reverseipdomain.com/?domain=$1"|jq|cut -d '"' -f2|grep "$1" | sort -u| anew |sort >> $Dir/$1.txt
...
python3 /home/mohamed/Tools/Sublist3r/sublist3r.py -d $1 | grep "$1" |grep -v "Enumerating subdomains"|anew |sort >> $Dir/$1.txt 
